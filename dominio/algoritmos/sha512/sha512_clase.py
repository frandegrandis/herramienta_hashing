...
from helpers.operaciones_bit_a_bit import rotar_derecha_bitarray, bitarray_de_numero, rotar_derecha
...
class SHA512(Algoritmo):
    ...
    def update(self, string):
        self.valor_a_hashear = string
        return

    def hexdigest(self):
        return self.sha512f(self.valor_a_hashear)

    def sha512f(self, message: str) -> str:
        ...
        # El 112, es el largo del mensaje menos el padding. Un mensaje tiene 128 bytes -> 128 -16 (del largo) -> 112
        # El 119 sale de que yo le agrego
        ...
        for chunk_start in range(0, len(message_array), 128):
            ...
            for i in range(80):
                temp1 = h + gamma1(e) + ch(e, f, g) + round_constants[i] + w[i]
                temp2 = gamma0(a) + maj(a, b, c)

                h = g
                g = f
                f = e
                e = suma_modular(d, temp1, modulo=2 ** 64)
                d = c
                c = b
                b = a
